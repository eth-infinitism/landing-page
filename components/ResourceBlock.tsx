  ...
  <div>
    ...
    <ul>
      {resources.map(({ name, url }) => (
        <Link href={url} key={url} target="_blank" className="max-w-fit list-item list-disc ml-5 no-underline text-blue-600 visited:text-purple-600 hover:underline hover:decoration-2 hover:text-800">
          {name}
        </Link>
      ))}
    </ul>
  ...
  return (
    <section>
      ...
      <div className="flex flex-col gap-y-3">
        {sections.map(item => (
          <ResourceSection {...item} key={item.sectionName}/>
        ))}
      </div>
    </section>
  );
