...
Because the Account Factory contract address is the same on every chain, we can rely on it to deploy our smart contract code on all networks under the same address too.

`salt`
For an ERC-4337 account, the salt parameter is the first nonce value. This is most likely 0.

`initCodeHash`
The initCode, passed along as a field of the UserOperation, is the smart contract code and arguments used for initializing it. It is hashed using keccak256 to derive the initCodeHash.
